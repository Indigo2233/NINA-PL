...
public sealed class TrainedDarkExposureInstruction : ISequenceItem
{
    public string Name { get; set; } = nameof(TrainedDarkExposureInstruction);
    ...
    public int DarkCount { get; set; } = 10;

    /// <summary>When true, flat panel cover stays closed during darks; when false, cover opens (e.g. thermal sky dark).</summary>
    ...
    public async Task ExecuteAsync(SequenceContext context, CancellationToken ct)
    {
        ...
        await flat.ToggleLightAsync(false).ConfigureAwait(false);
        ...
        int n = Math.Max(1, DarkCount);
        Logger.Info(
            "Trained dark: {0} frames, {1}s, Gain={2}, Offset={3}, Filter={4}, KeepClosed={5}",
    ...
    public TimeSpan GetEstimatedDuration() =>
        TimeSpan.FromSeconds(Math.Max(0.1, (ExposureSeconds + 1.0) * Math.Max(1, DarkCount)));
}
