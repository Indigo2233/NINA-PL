...
public sealed class WaitIfMoonAltitudeInstruction : ISequenceItem
{
    public string Name { get; set; } = nameof(WaitIfMoonAltitudeInstruction);
    ...
    public double AltitudeThreshold { get; set; }
    ...
    public async Task ExecuteAsync(SequenceContext context, CancellationToken ct)
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            Logger.Info("WaitIfMoonAltitude: Moon alt={0:F2}° (threshold={1}, {2}), waiting 30s", alt, AltitudeThreshold, Comparator);
