...
public sealed class WarmCameraInstruction : ISequenceItem
{
    public string Name { get; set; } = nameof(WarmCameraInstruction);
    ...
    public async Task ExecuteAsync(SequenceContext context, CancellationToken ct)
    {
        ...
        Logger.Info("Warming camera to {0}°C over {1} min", TargetTemperature, DurationMinutes);
